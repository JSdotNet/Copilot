...
- `automation-*` — scheduled or human-started sweeps. Never fired mid-task.
- `workflow-*` — spawn worker sessions and drive the `Workflow` tool. Deliberate and expensive.
...
Before marking anything user-invoked, grep the agents and hook prompts for its name. A skill an agent dispatches to must stay model-invoked, however deliberate it looks — the invariant above is not negotiable, and a mention that only records provenance (the orchestrator naming `automation-bug-fix` as the skill that routed a run) does not count as a dispatch.
...
- [ ] Every `automation-*`, `workflow-*`, and `knowledge-base:from-spec-*` skill sets `disable-model-invocation: true`.
- [ ] No `orch-*`, `phase-*`, `create-*`, `product-owner:write-*`, or `knowledge-base:to-spec-*` skill sets it.
