...
4. If artifacts exist (plans, partial analyses, checklists), store them in the repository's configured working-artifacts location and reference the path in the handoff context.
...
- For planning or implementation handoffs, include:
  ...
  - Remaining steps
  - Paths to related saved artifacts
- Partial outputs (for example implementation plans) must be saved to a stable, repository-approved location rather than temporary or ad hoc locations.
...
- [ ] Relevant partial artifacts were saved and referenced in the handoff.
