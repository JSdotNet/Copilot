...
- Keep project-wide plugin composition conventions in `create-plugin.md` instead of duplicating them here.
...
- Apply alongside `create-plugin.md` when a canvas extension is bundled inside a plugin package.
...
- Follow [spec-conciseness.md](spec-conciseness.md) for pruning.

## Validation Checklist

- [ ] Extension file is named `extension.mjs` and uses ES module syntax.
- [ ] Canvas `id` and action `name`s are unique and kebab-case.
- [ ] `open` reuses existing per-instance state instead of duplicating it.
- [ ] `onClose` releases all per-instance resources.
