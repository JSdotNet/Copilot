...
| Path-specific instructions | `instructions/*.md` plus a loader per host | Rules that apply only to specific file paths or asset types | Both — the body is host-neutral; `.github/instructions/*.instructions.md` carries `applyTo`, `.claude/rules/*.md` carries `paths` (repository only) |
