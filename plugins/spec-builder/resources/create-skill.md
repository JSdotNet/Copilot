...
- Reference contracts in `resources/` by relative path. Neither host auto-applies a file
  from inside a plugin, so the explicit reference is what loads the guidance — in both.
