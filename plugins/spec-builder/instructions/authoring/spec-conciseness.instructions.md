...
- Canonical sources: `docs/copilot/claude-code-compatibility.md` for dual-host rules, root
  `.github/instructions/` for repository-wide standards, and the matching
...
- Known no-ops here: "keep content in English" (set repository-wide in
  `.github/instructions/agent-language-and-tone.instructions.md`), "remove ambiguity",
  "validate consistency", "be thorough".

## Size Budgets

The budget is the trigger for a disclosure decision, not a hard limit. Count authored body
lines; generated frontmatter such as an agent `tools` list does not count.
