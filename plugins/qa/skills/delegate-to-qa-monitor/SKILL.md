...
- If the QA session is running inside the GitHub Copilot App and genuine parallel
  ...
  (`create_session`/cross-session messaging), as a flow that owns a QA validation phase
  does inline in that phase — that capability is not available to a plugin agent's own tool
  set.
