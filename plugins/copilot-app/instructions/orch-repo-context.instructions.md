...
description: Defines the optional .github/copilot-orch-context.md convention a consuming repository uses to declare how its application starts, where to validate it, and how deep QA should go, so orchestrations do not have to guess or ask.
---

# Orchestration Repo Context (Orchestration-Owned)
...
- Keep the file strictly about **runtime and validation context**. Model choice stays in
  `.github/copilot-model-selection.md` (see
...
The MCP servers this repository relies on during orchestration, named by their server IDs.
...
The `orch-*` skills this repository defines itself under `.github/skills/`, which are
invisible to the plugin and to the plugin-global routing hook. One bullet per skill, naming
the skill and the task category it owns — for example
`` `orch-backlog` — backlog and work-item orchestration ``.

A repo-native skill **takes precedence** over the plugin-provided skill for the categories it
covers. Declaring them here lets the orchestrator learn them from the file it already reads
...
- **Optional, never required.** A missing file changes nothing; orchestrations behave as
  they do today.
...
- **No model IDs.** This file must not pin a model, a model family, or a tier. Model choice
  belongs to `.github/copilot-model-selection.md` and the categories in
...
- **Single source of truth.** Do not copy this convention's prose into `orch-*/SKILL.md`
...
- [ ] The orchestrator reads the file once per run and persists the values it uses, rather
      than re-reading it per stage.
- [ ] A missing or malformed file leaves current behavior unchanged.
- [ ] `## Repo-Native Orchestration Skills`, when present, names skills that actually exist
      under the repository's `.github/skills/`.
