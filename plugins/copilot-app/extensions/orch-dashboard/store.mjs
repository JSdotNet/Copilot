// Persistence layer for orch-dashboard runs.
//
// Each run is stored as one JSON file under `<baseDir>/<runId>.json`. `baseDir`
// is resolved by the caller (extension.mjs) to the session workspace path so
// runs live alongside other session artifacts and are cleaned up with the
// session. Reads/writes are simple whole-file JSON round-trips; run counts
// per session are small (tens, not thousands) so this needs no indexing.
