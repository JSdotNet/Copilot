---
name: orch-adr
description: 'Orchestrate ADR creation with the Claude Code orchestration dashboard. Uses the architecture:architect agent for decision documentation plus `jsdotnet-guidelines-mcpserver` to retrieve relevant project guidance and existing ADR context first.'
---

# Orchestrate Architectural Decision Record

Execute an ADR workflow with the Claude Code orchestration dashboard with upfront MCP-based guidance retrieval and architecture-agent drafting.
...
- During **ADR Drafting**, also open/update the `render_markdown` dashboard tool with the
  drafted ADR content, per `instructions/dashboard-usage.instructions.md`. Optional; skip
...
Source skill location: `plugins/claude-desktop/skills/orch-adr/SKILL.md`
