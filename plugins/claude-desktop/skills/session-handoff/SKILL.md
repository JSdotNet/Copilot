...
next session pays a second time for everything this one learned, and an orchestration run
left behind looks merely idle.

## When Not to Hand Off

- The run is one short stage from `finish_run`. Finish it, and note the context pressure in
  the Summary.
...
- **Verification**: the exact build, test, and run commands already used, and their last
  known result. Include the app URL or dashboard link if one was running.
...
- **Evidence and artifact paths** as absolute paths — screenshots, reports, plans under
  `.wip/`.
- **Origin metadata** when the work came from an issue: repository, issue number, issue URL.
  Carry it forward so the next session can still report back to the issue (it passes this as
...
- State explicitly **what does not travel**: this branch, this worktree, the uncommitted
  diff, the running application, and the orchestration run — run state is stored per project,
  so the target repository gets its own.
...
1. `set_run_context` with `changeKind`, and `approval` plus the user's wording when Personal
   Validation has already decided. Conversation memory does not survive the handoff; the run
   JSON does.
...
Origin: <issue URL, or "ad-hoc request">
...
- The orchestration run marked handed off (same repo) or closed with a summary (cross-repo),
  when a run exists.
