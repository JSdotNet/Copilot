...
(`instructions/workflow-issue-sweep-contract.instructions.md`) for the directory layout and
...
5. Lead with the line that decides whether the reader keeps reading:
   ...
   ```text
   Overnight sweep jsdotnet-copilot-20260826-0600: 5 issues worked — 2 pull requests ready,
...
8. Render the brief in chat. When output includes `file`, also write it to
   `<sweep dir>/brief.md`.
9. When the `orch-dashboard` MCP server is available, call `render_markdown` with the brief so
   it opens as a readable document rather than scrolling past in the transcript.
...
- Optionally `brief.md` in the sweep directory, and a rendered document on the dashboard.
...
- Pair it with `pr-merge-ready` to move the pull requests in section ③ once you have read them.
