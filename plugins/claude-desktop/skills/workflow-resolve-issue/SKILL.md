...
This is the unattended lane. `start-session-from-issue` is the interactive one: it routes to
an `orch-*` orchestration that stops at Personal Validation and leaves the pull request to
...
Sweep State Contract** (`instructions/workflow-issue-sweep-contract.instructions.md`) for the
...
1. **One issue per run.** A run selects one issue, claims it, resolves it, and ends — never
   ...
   `workflow-issue-sweep`'s job, and it achieves it by spawning one worker session per issue,
...
## Relationship to the `orch-*` Contract

This skill is **not** an `orch-*` orchestration, and the shared instruction files under
`instructions/` (`applyTo: 'skills/orch-*/SKILL.md'`) do not govern it. Two differences are
deliberate:

| `orch-*` contract | This skill |
| --- | --- |
| Personal Validation gates the pull request; `approval` must be `approved` first | No user turn exists, so the **pull request is the review surface**. Nothing merges without you. |
| Never run an orchestration as a sub-agent | This skill never invokes an `orch-*` skill. The resolution stages are implemented directly in the workflow script, so no orchestration is pushed into a sub-agent. |
...
whose conclusion is small belongs in a sub-agent, and the dashboard reporting contract.
...
- Sweep context, when spawned by `workflow-issue-sweep`: the absolute sweep directory and the
  sweep id. Absent when run standalone.

## Skill Dependencies

- **`create-pull-request`** (this plugin) — used in Phase 4a when it is installed, so the pull
  request picks up the repository's PR template, labels, and reviewer conventions. When it is
  absent, fall back to the `gh pr create` call in that phase and say so in the summary.

- **`session-handoff`** (this plugin) — writes the brief the park route leaves behind in
  Phase 4b. When absent, write the equivalent brief by hand as that phase describes.
...
18. Write a handoff brief with `session-handoff` (target: `same-worktree`, reason: the
    `parkReasons` from the workflow). When that skill is unavailable, write the equivalent
    brief yourself to `~/.claude/handoffs/<issue>-<slug>.md`: the objective, the issue link,
...
```text
Every weekday at 08:00, run workflow-resolve-issue for JSdotNet/Copilot with label "ready"
...
dashboard calls and continue through standard chat interaction. Follow the shared
**Dashboard Reporting Contract** in `instructions/orch-dashboard-contract.instructions.md`
for the tool cadence and for resolving the tool prefix.

- Open the dashboard per the shared contract, then call `start_run` with
  `skillId: "workflow-resolve-issue"` and these stages: Select and Claim One Issue, Provision
...
- Before each phase, `update_stage` with `status: "in_progress"`; after each,
  ...
  summary.
- The workflow's own phases appear in `/workflows`, not on the dashboard — its agents are
  sub-agents and never call dashboard tools. Record the workflow's returned `outcome` and
  counts as the Run the Resolution Workflow stage output.
...
- **Concurrency is safe but not free.** Two runs claim different issues and work disjoint
  ...
  schedule, or give each run its own ports via `.claude/orch-context.md`.
...
- **No QA validation phase.** The workflow verifies with the unit suite only. Browser-level
  ...
  of reaching a pull request. Resume the parked worktree and run `phase-qa-validation` there,
  or work the issue through `start-session-from-issue` and `orch-feature` from the start.
...
- `start-session-from-issue` — the interactive counterpart: same single-issue pickup, routed
  to an `orch-*` orchestration in your session, stopping at Personal Validation.
- `automation-bug-fix` — the same interactive pickup narrowed to `bug` issues.
- `pr-merge-ready` — takes the pull request this skill opens to merge-ready, one per pass.
- `workflow-issue-sweep` — spawns this skill as a worker session, one per issue, up to five
  at a time.
- `workflow-morning-brief` — reports what every worker did, from the result files this skill
  writes.
- `session-handoff` — writes the brief the park route leaves behind.
- `create-pull-request` — used by Phase 4 when installed, so the PR follows the repository's
  own template and conventions.
