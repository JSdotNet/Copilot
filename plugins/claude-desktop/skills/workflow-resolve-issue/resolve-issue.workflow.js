export const meta = {
  ...
  whenToUse:
    'Invoked by the workflow-resolve-issue skill after it has selected, claimed, and provisioned a worktree for a single GitHub issue. Not a standalone entrypoint: it expects args.worktree to exist and args.issue to be claimed.',
  phases: [
...
if (!args || !args.worktree || !args.issue || !args.issue.number || !args.branch) {
  throw new Error(
    'resolve-issue.workflow.js requires args.worktree (absolute path), args.branch, and ' +
      'args.issue.number. Invoke it through the workflow-resolve-issue skill, which claims ' +
  ...
  properties: {
    ...
    escalate: {
      ...
      properties: {
        ...
        routeTo: {
          type: 'string',
          description: 'orch-adr | orch-architecture | orch-blueprint | orch-tdr | orch-create-service | orch-domain',
        },
      },
    },
    unresolvedQuestions: {
...
if (scope.escalate && scope.escalate.needed) {
  ...
  return {
    ...
    routeTo: scope.escalate.routeTo || 'orch-architecture',
