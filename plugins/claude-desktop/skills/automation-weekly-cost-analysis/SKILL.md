...
Read the token usage the `orch-dashboard` MCP server recorded for each orchestration run
...
1. Call `list_runs` on the `orch-dashboard` MCP server and keep the runs whose `startedAt`
...
- This skill reads only what the dashboard already recorded, so it is a no-op when no
  orchestration runs fall inside the window.
