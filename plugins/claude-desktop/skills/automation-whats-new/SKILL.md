...
- State file path (optional; default: `.claude/state/whats-new/state.json` relative to the
...
Maintain one JSON file (path from Inputs, default `.claude/state/whats-new/state.json`) with
one entry per repo:
