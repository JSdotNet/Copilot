...
description: >
  ...
  select one, claim it, route it to the orch-* skill that matches its type, and run that
  orchestration here. One issue per run, no extra sessions. Use when: picking up an issue
...
a single issue, claim it, decide which `orch-*` orchestration its type calls for, and run
that orchestration **in this session** with the issue context and origin metadata baked in.

This is the generic counterpart to `automation-bug-fix`: any filter, any issue type, routed
to the matching orchestration rather than always `orch-bug`.

## One Issue Per Run

This skill starts **this** session's work from one issue. It does not prepare work for other
sessions, and it never spawns an agent to run the orchestration.

An `orch-*` run needs its own session: it must be able to ask about what is ambiguous, hold
the Personal Validation gate, and own its dashboard run. `AskUserQuestion` is foreground-only
and a sub-agent has no user turn to wait for — see **Session Ownership** and **Sub-Agent
Constraints** in `instructions/orch-execution-model.instructions.md`. Scoping a run to one
issue is what makes that work: this session is the owner session, so the orchestration is
...
- Plan-first: `true` (default on an interactive run — the orchestration proposes its plan and
  waits for approval before implementing) or `false` (the orchestration's own stage gating
...
7. **Confirmation depends on whether a user is there.**
   ...
   - **Unattended run** (scheduled routine, no user turn available): proceed without
     ...
     the orchestration still stops at Personal Validation before any pull request.

### Phase 4 — Route to an Orchestration

8. Decide which `orch-*` skill the issue's type calls for. Read the issue body and labels,
   not the labels alone — a mislabelled issue routes on what it actually asks for:

   | Issue is about | Orchestration |
   |---|---|
   | A defect in existing behavior | `orch-bug` |
   | New or changed feature behavior, including small UI tweaks | `orch-feature` |
   | A new module, or carving one out of an existing area | `orch-create-module` |
   | A new service, or extracting one | `orch-create-service` |
   | Folder moves, project/solution layout, test placement | `orch-structure` |
   | Dependency or package updates | `orch-update-packages` |
   | An Aspire version upgrade | `orch-aspire-update` |
   | Architecture, ADR, arc42, blueprint, TDR | `orch-architecture`, `orch-adr`, `orch-arc42`, `orch-blueprint`, `orch-tdr` |
   | Anything no skill above covers — tooling, CI, scripting, housekeeping | `orch-fallback` |

   This mirrors the routing the plugin's `SessionStart` hook installs; a repository may ship
   its own `orch-*` skills under `.claude/skills/`, and those take precedence for the
   categories they cover. `orch-fallback` is the last resort, not an escape hatch.

   State the routing decision and its reason before acting on it. When the issue is too
   ambiguous to route, ask (interactive) or route to `orch-fallback` and say so (unattended).
...
10. Run the routed orchestration in **this session** with the context below. Pass the GitHub
...
11. When plan-first is enabled, the orchestration's scope-discovery stage proposes its plan —
    ...
    answered. Nothing is lost by continuing: the orchestration still stops at Personal
...
12. **Never spawn an agent to run the orchestration**, and never pick up a second issue in
    this run.
...
13. Output a summary:
    ...
    | Routed to | `orch-feature` (feature behavior, labelled `feature`) |
...
- Open the dashboard per the shared contract, then call `start_run` with
  `skillId: "start-session-from-issue"` and these stages: Fetch Matching Issues, Filter Out
  Work Already In Flight, Select One Issue, Route to an Orchestration, Claim and Run,
  Summary.
...
- The orchestration in Phase 5 opens its own run, with this run's `githubIssue` metadata
...
- `automation-bug-fix` — the same single-issue pickup narrowed to `bug` issues, always routed
  to `orch-bug`, ranked by severity.
- `pr-merge-ready` — takes the pull request behind the finished work to merge-ready, one PR
  per pass.
- `session-handoff` — hand this session's in-flight run to a fresh session when its context
  fills, rather than starting the issue over.
