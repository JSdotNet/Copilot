---
name: orch-feature
description: 'Orchestrate feature work end to end with the Claude Code orchestration dashboard — from an ad-hoc request or an approved specification through implementation, validation, review, and personal approval. Use for new features, incremental changes to existing features, and small UI tweaks; missing scope, acceptance criteria, or architecture context is derived in Stage 0 rather than being a reason to skip orchestration.'
---

# Orchestrate Feature Development

Execute a feature implementation workflow — from an ad-hoc request or approved
requirements and architecture — through local validation using dashboard.
...
- During **Specification & Architecture Intake**, optionally open/update
  the `render_markdown` dashboard tool or the `render_diagram` dashboard tool with the
  provided specification or architecture artifacts, per
  `instructions/dashboard-usage.instructions.md`.
...
Source skill location: `plugins/claude-desktop/skills/orch-feature/SKILL.md`
