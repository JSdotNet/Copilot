...
description: 'Update all outdated NuGet packages in a .NET solution, check installed Claude Code plugins for newer versions, and open a PR with the changes. Handles Central Package Management, Aspire integration upgrades, and post-update build and test verification.'
...
2. Check installed Claude Code plugins for newer versions: compare the `version` of each
   installed plugin against the `version` in its marketplace's `.claude-plugin/marketplace.json`
   (for this repository's plugins, the file at the marketplace repo root). Report any that
...
12. Commit all changes with message:
    ...
    - Claude Code plugins: <n> behind (reported, not updated)
    ```
