---
name: orch-arc42
description: 'Orchestrate arc42 architecture documentation with the Claude Code orchestration dashboard. Uses the architecture:architect agent for section drafting and `jsdotnet-guidelines-mcpserver` for guideline and ADR grounding before governed asset changes.'
---

# Orchestrate arc42 Documentation

Execute an arc42 documentation workflow with the Claude Code orchestration dashboard while keeping the architect agent independent and moving guideline retrieval into the orchestration layer.
...
- During **Section Drafting**, also open/update the `render_markdown` dashboard tool with
  the drafted arc42 section content, per `instructions/dashboard-usage.instructions.md`.
...
Source skill location: `plugins/claude-desktop/skills/orch-arc42/SKILL.md`
