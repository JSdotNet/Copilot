...
description: 'Orchestrate creating a new module in an existing project with the Claude Code orchestration dashboard — from an ad-hoc "we need a module for X" request or an approved module specification through planning, implementation, testing, local run, and monitoring. Use for new modules and for carving an existing area into a module; missing specification, boundaries, or architecture context is derived in Stage 0 rather than being a reason to skip orchestration.'
---
...
- During **Specification Intake**, also open/update the `render_markdown` dashboard tool
...
Source skill location: `plugins/claude-desktop/skills/orch-create-module/SKILL.md`
