---
name: orch-bug
description: 'Orchestrate bug resolution end to end with the Claude Code orchestration dashboard — from an ad-hoc "this is broken" report or a triaged issue through reproduction, root cause analysis, TDD fix implementation (test first), verification, and local runtime validation. Missing reproduction steps, severity, or root cause context is derived in Stage 0 rather than being a reason to skip orchestration.'
---
...
- During **Bug Intake & Reproduction**, also open/update the `render_markdown` dashboard tool
  with the drafted bug report content, per `instructions/dashboard-usage.instructions.md`.
...
Source skill location: `plugins/claude-desktop/skills/orch-bug/SKILL.md`
