...
description: 'Orchestrate .NET Aspire upgrades with a plan-first workflow with the Claude Code orchestration dashboard. Creates and refines an upgrade plan, performs staged updates, enables new Aspire features, and validates runtime behavior with recorded results.'
---
...
- During **Plan Refinement**, also open/update the `render_markdown` dashboard tool with
  the refined upgrade plan, per `instructions/dashboard-usage.instructions.md`. Optional; skip
...
Source skill location: `plugins/claude-desktop/skills/orch-aspire-update/SKILL.md`
