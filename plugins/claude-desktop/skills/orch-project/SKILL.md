...
Automate the complete project scaffolding workflow for an existing, configured repository using Claude Code dashboard, including automated local validation and testing.

> **Note:** This skill assumes your repository already exists and is configured (README, repository instructions, MCP servers, branch protection, templates). Use `orch-repo` first to create and configure the repository, then use this skill to scaffold the development project inside it.
>
...
- **Create `.claude/settings.json`** for repository-level Claude Code configuration
- **Create `.claude/orch-context.md`** so later orchestrations know how to start and
  validate this project — see `instructions/orch-repo-context.instructions.md` and the
  template at `resources/claude-orch-context-template.md`. A repository with nothing to run
...
2. **QA Validation** — new runnable scaffold, so run QA validation with capture: start the Aspire
   ...
   `.claude/orch-context.md` when Stage 1 wrote one) and database connectivity
...
- `.github/` directory fully initialized with instructions and workflows, plus `.claude/` configuration.
...
- During **Specification & Architecture Intake**, also open/update `render_markdown`
  ...
  the `render_diagram` dashboard tool with any accompanying Mermaid diagrams, per
...
Source skill location: `plugins/claude-desktop/skills/orch-project/SKILL.md`
