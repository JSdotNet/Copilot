...
- During **Specification Intake**, also open/update `render_markdown`
  ...
  **Implementation Planning**, open/update `render_markdown` with the design
  documentation and the `render_diagram` dashboard tool with any accompanying Mermaid
  diagrams, per `instructions/dashboard-usage.instructions.md`. Optional; skip gracefully if
  not installed.
...
Source skill location: `plugins/claude-desktop/skills/orch-create-service/SKILL.md`
