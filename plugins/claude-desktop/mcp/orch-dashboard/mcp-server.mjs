...
// stages carrying status, output, links, QA scenarios and monitoring findings — and the
// rendering, reporting and insight modules are shared verbatim with the Copilot version.
// Only the transport differs:
//
//   Copilot CLI                        Claude Code
//   canvas panel in the host           browser page served on 127.0.0.1
//   canvas actions                     MCP tools (this file)
//   session telemetry events           hook events (telemetry-hook.mjs)
//
// The same server also replaces the `diagram-canvas` and `markdown-canvas` extensions:
// `/mermaid` and `/markdown` serve their original pages verbatim, driven by the
// render_diagram and render_markdown tools.
//
// Everything is served on 127.0.0.1 with an ephemeral port, exactly like the Copilot
// extension's canvas server. There is no authentication: reaching it already requires
...
const SERVER_NAME = "orch-dashboard";
const SERVER_VERSION = "0.1.0";
const SUPPORTED_PROTOCOLS = ["2025-06-18", "2025-03-26", "2024-11-05"];

// MCP Apps (SEP-1865). A host that implements this extension renders the pages below inline
// in the conversation instead of the user opening them in a browser — the closest equivalent
// of the Copilot canvas panel this dashboard was ported from. Hosts that do not implement it
// ignore the resources entirely and the HTTP surface stays the way in.
...
async function appPageHtml(page) {
    ...
    const preamble =
        `<script>window.__ORCH_HTTP_ORIGIN__ = ${JSON.stringify(origin || "")};</script>\n` +
        `<script>\n${bridge}\n</script>\n`;
...
const tools = [
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                originalPrompt: { type: "string", description: "Original user prompt or request text that started this orchestration run." },
                promptHistory: {
                    type: "array",
        ...
        handler: async (input) => {
            const { skillId, title, stages, originalPrompt, promptHistory, githubIssue, changeKind, resume } = input;
    ...
    },
    {
        name: "get_view",
        description:
            "Read the current diagram or document view: its title, source/content, explanation, and history depth. Mostly used by the rendered app to refresh itself.",
        inputSchema: {
            type: "object",
            properties: { viewer: { type: "string", enum: ["mermaid", "markdown"] } },
            required: ["viewer"],
        },
        handler: async ({ viewer }) => {
            const target = viewers[viewer];
            if (!target) throw new ToolError("viewer must be mermaid or markdown");
            return target.currentView ? viewPayload(target) : {};
        },
    },
    {
        name: "pop_view",
        description: "Go back one step in a viewer's history, undoing the most recent push. Returns the restored view, or null when the history is empty.",
        inputSchema: {
            type: "object",
            properties: { viewer: { type: "string", enum: ["mermaid", "markdown"] } },
            required: ["viewer"],
    ...
    },
    {
        ...
        _meta: { ui: { resourceUri: "ui://orch-dashboard/diagram.html" } },
    ...
    },
    {
        ...
        description:
            "Render or update a Markdown document in the document viewer — the replacement for the Copilot markdown-canvas. Render the same Markdown that was written to the file artifact; the file stays the source of truth.",
        inputSchema: {
            type: "object",
            properties: {
        ...
        _meta: { ui: { resourceUri: "ui://orch-dashboard/document.html" } },
...
process.stdin.on("data", (chunk) => {
    ...
    while ((newline = buffer.indexOf("\n")) >= 0) {
        ...
        handleMessage(msg).catch((err) => {
            process.stderr.write(`orch-dashboard: ${String((err && err.stack) || err)}\n`);
        });
    }
});
