// Shared state resolution for the orch-dashboard MCP server and its telemetry hook.
//
...
//
//   runs/<runId>.json          one orchestration run (see store.mjs)
...
export function stateDir() {
    if (process.env.ORCH_DASHBOARD_STATE_DIR) {
        return path.resolve(process.env.ORCH_DASHBOARD_STATE_DIR);
    }
    ...
    return path.join(home, "orch-dashboard", `${slug}-${hash}`);
}
