...
try {
    ...
    const handed = await call("set_run_context", {
        runId,
        handoff: true,
        handoffNote: "Export endpoint written, tests not run yet. Resume with: orch-feature export endpoint",
    });
    ...
    const other = await call("start_run", { skillId: "orch-bug", title: "Abandoned", stages: [{ name: "Reproduce" }] });
    ...
    const fresh = await call("start_run", { skillId: "orch-bug", title: "Abandoned", stages: [{ name: "Reproduce" }] });
