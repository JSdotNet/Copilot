...
// inside an MCP App. `render.mjs` and the two viewer pages are byte-identical to the GitHub
// Copilot canvas versions, and stay that way: this bridge adapts the environment to them
...
//   2. Replaces `fetch` for the pages' own JSON routes, answering them with `tools/call`
//      results instead. `/api/runs` becomes `list_runs`, `/mermaid/api/state` becomes
//      `get_view`, and so on.
...
(function () {
  ...
  var ORIGIN = window.__ORCH_HTTP_ORIGIN__ || "";
  var POLL_MS = 2000;
  ...
  var ROUTES = [
    ...
    { method: "GET", pattern: /^\/(mermaid|markdown)\/api\/state$/, handler: function (m) { return callTool("get_view", { viewer: m[1] }); } },
    { method: "POST", pattern: /^\/(mermaid|markdown)\/api\/back$/, handler: function (m) { return callTool("pop_view", { viewer: m[1] }); } },
  ];
  ...
  function BridgeEventSource(url) {
    ...
    if (viewer) {
      poll = function () {
        return callTool("get_view", { viewer: viewer[1] }).then(function (view) {
