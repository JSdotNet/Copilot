...
| Skill | Runs in | Owns |
| --- | --- | --- |
| `workflow-issue-sweep` | The routine session, held open through triage, dispatch, closure, the wait, and the brief | Triage, conflict detection, dispatching workers, the closure approval, and its own final report |
| `workflow-resolve-issue` | One independent `claude --bg` session per worker, each in its own worktree | One issue: resolve, then PR or park |
| `workflow-morning-brief` | Never invoked by the other two — a standalone skill a human runs to re-read a sweep later | The report *format* `workflow-issue-sweep` follows for its own brief |
...
~/.claude/issue-sweep/<sweepId>/
  ...
  brief.md                # the report, written by workflow-issue-sweep itself once workers finish
```
...
- `briefWrittenAt` is set by `workflow-issue-sweep` itself, in the same update that records
  ...
  `workflow-morning-brief` is how you find out which.
...
`workflow-issue-sweep` launches one **independent background session per picked issue** with
...
- **Run this from a checkout of the target repository.** `claude --bg` roots the new session
  at the current working directory; `workflow-resolve-issue`'s own Phase 2 does the actual
...
session to do it later. `create_scheduled_task` does not appear anywhere in this contract.
...
way; if a sweep goes quiet, run `workflow-morning-brief` by hand against its sweep directory
...
`workflow-issue-sweep` polls every `waitPollMinutes` (default `5`) for up to `maxWaitMinutes`
(default `90`):
...
- **The wait ends** when every issue has a result file, when every remaining one has been
  ...
  as a failure; a later hand-run of `workflow-morning-brief` will show it resolved.

Once the wait ends, `workflow-issue-sweep` writes `brief.md` and prints it in chat itself,
following `workflow-morning-brief`'s own Phase 1 step 4 (refresh live PR/issue state), Phase 2
...
`workflow-morning-brief` as a skill.

## Rules That Keep a Sweep Honest

- **Claim before dispatching.** `workflow-issue-sweep` labels each picked issue
