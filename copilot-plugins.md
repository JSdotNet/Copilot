...
| `claude-desktop` | `0.8.0` | `plugins/claude-desktop` | `.mcpb` extension (Claude Desktop) or `/plugin install claude-desktop@jsdotnet-copilot` (Claude Code) | Claude sibling of `copilot-app`: the same orchestration skills plus a Claude-only pull request lane (`create-pull-request`, `update-pr-branch`, `fix-pr-checks`, `pr-merge-ready`), with the canvas replaced by an `orch-dashboard` MCP server that renders inline as an MCP App in Claude Desktop (run dashboard, Mermaid viewer, Markdown viewer). Its automation skills deliberately diverge from `copilot-app`: each `orch-*` run takes a single issue or pull request and carries it through in its own session. Adds a `workflow-*` lane driving the `Workflow` tool instead of an `orch-*` skill — `workflow-issue-sweep` triages the backlog for relevance and for collision with work in flight, proposes stale issues for closure, and spawns up to five one-time scheduled tasks as worker sessions; each worker (`workflow-resolve-issue`) resolves one issue in its own git worktree and routes on evidence gathered after the work, opening a pull request when every acceptance criterion is test-proved and parking the worktree with a handoff brief otherwise; `workflow-morning-brief` aggregates the results into one report. Claude-only. |
...
| `spec-builder` | `0.2.0` | `plugins/spec-builder` | `copilot plugin install JSdotNet/Copilot:plugins/spec-builder` | GitHub customization asset authoring for agents, instructions, plugins, and skills. |
