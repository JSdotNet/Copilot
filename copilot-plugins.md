...
| `claude-desktop` | `0.1.0` | `plugins/claude-desktop` | `.mcpb` extension (Claude Desktop) or `/plugin install claude-desktop@jsdotnet-copilot` (Claude Code) | Claude sibling of `copilot-app`: the same orchestration and automation skills, with the canvas replaced by an `orch-dashboard` MCP server that renders inline as an MCP App in Claude Desktop (run dashboard, Mermaid viewer, Markdown viewer). Claude-only. |
